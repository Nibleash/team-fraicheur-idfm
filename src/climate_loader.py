...
class ClimateDataLoader:
    ...
    def load_climate_tiles_from_s3(
        ...
        fs = s3fs.S3FileSystem(
            anon=True,
            client_kwargs={"endpoint_url": "https://object.files.data.gouv.fr"},
        )
        ...
        ds_jja_idf = (
            ds.sel(time=(ds.time.dt.year == year) & ds.time.dt.month.isin([6, 7, 8]))
    ...
    def convert_to_geodataframe(self, df: pd.DataFrame) -> gpd.GeoDataFrame:
        ...
        gdf_metric = gdf_points.to_crs("EPSG:2154")
        ...
        square_geometries = [
            ...
            for point in gdf_metric.geometry
        ]

        gdf_squares = gpd.GeoDataFrame(
            df, geometry=square_geometries, crs="EPSG:2154"
        ).to_crs("EPSG:4326")
